...
# =============================
# Enhanced Multi-VM Manager v6.0 (Next Generation)
# Direct Terminal Access - No SSH Required
# Optimized for: IDX GNU/Linux (Firebase Studio) & All Linux Systems
# Features: Direct Console, VM Edit, GPU Passthrough, Shared Folders, Monitoring,
#           Backup/Restore, Advanced Networking, USB Devices, Migration, No Limits
# =============================
...
# Detect IDX/Firebase Studio
if [[ -f /etc/os-release ]] && grep -q "IDX GNU/Linux" /etc/os-release 2>/dev/null; then
    IDX_ENV=true
    CLOUD_ENV="idx"
fi
...
check_dependencies() {
    ...
    if [[ ${#missing[@]} -ne 0 ]]; then
        ...
        # Detect package manager and provide appropriate installation command
        if command -v apt-get &>/dev/null; then
            echo "Install with: sudo apt-get update && sudo apt-get install -y qemu-system-x86 qemu-utils wget"
        elif command -v apt &>/dev/null; then
            echo "Install with: sudo apt update && sudo apt install -y qemu-system-x86 qemu-utils wget"
        elif command -v dnf &>/dev/null; then
            echo "Install with: sudo dnf install -y qemu-kvm qemu-img wget"
        elif command -v yum &>/dev/null; then
            echo "Install with: sudo yum install -y qemu-kvm qemu-img wget"
        elif command -v pacman &>/dev/null; then
            echo "Install with: sudo pacman -S qemu-full wget"
        elif command -v zypper &>/dev/null; then
            echo "Install with: sudo zypper install -y qemu-kvm qemu-tools wget"
        elif command -v apk &>/dev/null; then
            echo "Install with: sudo apk add qemu-system-x86_64 qemu-img wget"
        else
            echo "Please install: qemu-system-x86_64, qemu-img, wget (or curl)"
        fi
        exit 1
    fi
...
is_port_available() {
    ...
    # Try ss first (most modern systems)
    ...
    # Fallback to /proc/net (most reliable, works everywhere with /proc)
...
load_vm_config() {
    ...
    unset DISK_SIZE MEMORY CPUS GUI_MODE PORT_FORWARDS IMG_FILE SEED_FILE CREATED
...
decompress_if_needed() {
    ...
    case "$file" in
        *.xz)
            ...
            elif command -v pixz &>/dev/null; then
                pixz -d -k "$file" 2>/dev/null && mv "${file%.xz}" "$output"
            else
                print_status "ERROR" "xz decompression tool not found. Install with: sudo apt install xz-utils (or yum install xz)"
                return 1
            fi
            rm -f "$file"
            ;;
        *.gz)
            ...
            if command -v gunzip &>/dev/null; then
                ...
                gzip -dc "$file" > "$output" && rm -f "$file"
            else
                print_status "ERROR" "gzip decompression tool not found"
                return 1
            fi
            ;;
        *.bz2)
            ...
            if command -v bunzip2 &>/dev/null; then
                ...
                bzip2 -dc "$file" > "$output" && rm -f "$file"
            else
                print_status "ERROR" "bzip2 decompression tool not found"
                return 1
            fi
            ;;
        *.tar.xz)
            ...
            if tar --help 2>&1 | grep -q "\-J"; then
                ...
                xz -dc "$file" | tar -xf - -C "$(dirname "$output")" && rm -f "$file"
            else
                print_status "ERROR" "Cannot extract tar.xz - missing xz support"
                return 1
            fi
            local extracted
            extracted=$(find "$(dirname "$output")" -maxdepth 2 \( -name "*.qcow2" -o -name "*.img" \) 2>/dev/null | head -1)
...
create_cloud_init_iso() {
    ...
    # Error with platform-specific installation instructions
    print_status "ERROR" "No ISO creation tool available"
    echo ""
    if command -v apt-get &>/dev/null || command -v apt &>/dev/null; then
        echo "Install with: sudo apt install -y genisoimage"
    elif command -v dnf &>/dev/null; then
        echo "Install with: sudo dnf install -y genisoimage"
    elif command -v yum &>/dev/null; then
        echo "Install with: sudo yum install -y genisoimage"
    elif command -v pacman &>/dev/null; then
        echo "Install with: sudo pacman -S cdrtools"
    elif command -v zypper &>/dev/null; then
        echo "Install with: sudo zypper install -y mkisofs"
    elif command -v apk &>/dev/null; then
        echo "Install with: sudo apk add xorriso"
    else
        echo "Please install one of: genisoimage, mkisofs, or xorriso"
    fi
    return 1
}
...
build_qemu_base() {
    ...
    if $KVM_AVAILABLE; then
        ...
        # TCG emulation mode - check if machine type q35 is supported
        if qemu-system-x86_64 -machine help 2>/dev/null | grep -q "q35"; then
            QEMU_CMD+=(-machine type=q35,accel=tcg)
        else
            # Fallback to basic pc machine type
    ...
    # Add RNG device if /dev/urandom is available
...
build_qemu_foreground() {
    ...
    if [[ "${GUI_MODE:-false}" == "true" ]]; then
        QEMU_CMD+=(-vga virtio)
        # Try different display backends based on availability
        if [[ -n "${DISPLAY:-}" ]] && command -v gtk-launch &>/dev/null; then
            QEMU_CMD+=(-display gtk,gl=on)
        elif [[ -n "${DISPLAY:-}" ]]; then
            QEMU_CMD+=(-display gtk)
        else
            print_status "WARN" "No display available, switching to terminal console"
            QEMU_CMD+=(-display none)
            QEMU_CMD+=(-serial stdio)
        fi
    else
        # Direct terminal access - stdio mode
        QEMU_CMD+=(-display none)
        QEMU_CMD+=(-serial stdio)
...
start_vm() {
    ...
    echo
    print_status "INFO" "Starting VM: $vm_name (Direct Terminal Mode)"
    echo "════════════════════════════════════════════════════════"
    ...
    [[ -n "${PORT_FORWARDS:-}" ]] && echo "  Ports:    ${PORT_FORWARDS}"
    echo "════════════════════════════════════════════════════════"
    print_status "INFO" "Wait 30-60 seconds for boot to complete"
    print_status "INFO" "Press Ctrl+A then X to exit QEMU console"
    print_status "INFO" "Console will appear directly in this terminal"
    echo
...
start_vm_background() {
    ...
    if "${QEMU_CMD[@]}" 2>/dev/null; then
        ...
        if is_vm_running "$vm_name"; then
            ...
            [[ -n "${SPICE_PORT:-}" ]] && echo "  SPICE: localhost:${SPICE_PORT}"
            [[ -n "${PORT_FORWARDS:-}" ]] && echo "  Ports: ${PORT_FORWARDS}"
            echo "  Serial: socat - unix-connect:$VM_DIR/$vm_name.serial"
            print_status "INFO" "Use 'attach' command to connect to console"
        else
...
edit_vm() {
    ...
    while true; do
        ...
        echo "  4) Port Forwards (Current: ${PORT_FORWARDS:-none})"
        echo "  5) Network Mode (Current: ${NETWORK_MODE:-user})"
        echo "  6) Shared Folders (Current: ${SHARED_FOLDERS:-none})"
        echo "  7) GPU Passthrough (Current: ${GPU_PASSTHROUGH:-none})"
        echo "  8) USB Devices (Current: ${USB_DEVICES:-none})"
        echo "  9) VNC Port (Current: ${VNC_PORT:-disabled})"
        echo " 10) SPICE Port (Current: ${SPICE_PORT:-disabled})"
        echo " 11) Custom Arguments (Current: ${CUSTOM_ARGS:-none})"
        echo " 12) TPM Support (Current: ${TPM_ENABLED:-false})"
        echo " 13) Username (Current: ${USERNAME})"
        echo " 14) Password (Current: ${PASSWORD})"
        echo " 15) Hostname (Current: ${HOSTNAME})"
        echo " 16) GUI Mode (Current: ${GUI_MODE:-false})"
        ...
        case "$choice" in
            ...
            4)
                ...
                ;;
            5)
                echo "Network modes: user, bridge, tap, none"
            ...
            15)
                ...
                ;;
            16)
                read -p "$(print_status "INPUT" "Enable GUI mode? (true/false) [${GUI_MODE:-false}]: ")" new_gui
                GUI_MODE="${new_gui:-false}"
                print_status "SUCCESS" "GUI mode updated"
                ;;
            0)
                save_vm_config
...
quick_create_vm() {
    ...
    echo "  User:     $USERNAME"
    echo "  Password: $PASSWORD"
    echo "  Console:  Direct terminal access (no SSH needed)"
}
...
create_new_vm() {
    ...
    # Port Forwards
    GUI_MODE=false
    read -p "$(print_status "INPUT" "Port forwards (e.g., 8080:80,3000:3000) [optional]: ")" PORT_FORWARDS
...
show_system_info() {
    ...
    if command -v qemu-system-x86_64 &>/dev/null; then
        printf "  QEMU:        %s\n" "$(qemu-system-x86_64 --version 2>/dev/null | head -1 | awk '{print $4}' || echo 'installed')"
    fi
    printf "  VM Dir:      %s\n" "$VM_DIR"
    printf "  Environment: %s\n" "$CLOUD_ENV"
    [[ "$IDX_ENV" == "true" ]] && printf "  IDX Mode:    Enabled\n"

    # Show disk space in VM directory
    if [[ -d "$VM_DIR" ]]; then
        local disk_info
        disk_info=$(df -h "$VM_DIR" 2>/dev/null | tail -1 | awk '{print $4 " available of " $2}')
        [[ -n "$disk_info" ]] && printf "  Disk Space:  %s\n" "$disk_info"
    fi
    echo "════════════════════════════════════════════════════════"
    echo
}
...
Commands:
  ...
  start <vm>              Start VM with direct terminal console
  startbg <vm>            Start VM in background (daemon)
  attach <vm>             Attach to running VM console
  stop <vm>               Stop VM
  delete <vm>             Delete VM
  info <vm>               Show VM info
  edit <vm>               Edit VM configuration
  monitor <vm>            Monitor VM stats
  clone <vm>              Clone VM
  ...
  export <vm>             Export VM to OVA
  system                  System info
  oses                    List available OSes
  help                    Show this help

v6.0 Features:
  - Direct terminal console access (no SSH required)
  - Interactive console with Ctrl+A X to exit
  - Background mode with serial socket attachment
  - Simplified networking (no SSH port management)

Examples:
  $0 quick "Ubuntu 24.04 LTS (Noble)" myserver
  $0 start myserver                    # Direct console
  $0 startbg myserver                  # Background daemon
  $0 attach myserver                   # Connect to running VM
...
main_menu() {
    while true; do
        ...
        echo "╔═════════════════════════════════════════════════════════╗"
        echo "║                    Main Menu v6.0                       ║"
        echo "║              Direct Terminal - No SSH Needed            ║"
        echo "╠═════════════════════════════════════════════════════════╣"
        echo "║  1) Create VM          2) Start (console)               ║"
        echo "║  3) Start (bg)         4) Attach Console                ║"
        echo "║  5) Stop VM            6) VM Info                       ║"
        echo "║  7) Edit VM            8) Clone VM                      ║"
        echo "║  9) Monitor VM        10) Snapshots                     ║"
        echo "║ 11) Resize Disk       12) Delete VM                     ║"
        echo "║ 13) Backup VM         14) Restore Backup                ║"
        echo "║ 15) List Backups      16) Export VM                     ║"
        ...
        case "$choice" in
            ...
            4)
                if [[ $count -gt 0 ]]; then
                    local vm
                    vm=$(select_vm "Attach to VM" "${vms[@]}")
                    if [[ -n "$vm" ]]; then
                        if is_vm_running "$vm"; then
                            load_vm_config "$vm"
                            if [[ -S "$VM_DIR/$vm.serial" ]]; then
                                print_status "INFO" "Connecting to $vm console (Ctrl+C to detach)..."
                                socat - "unix-connect:$VM_DIR/$vm.serial"
                            else
                                print_status "ERROR" "Serial socket not available"
                            fi
                        else
                            print_status "ERROR" "VM is not running"
                        fi
    ...
    case "$1" in
        ...
        attach)
            ...
            if ! is_vm_running "$1"; then
                print_status "ERROR" "VM '$1' is not running"
                exit 1
            fi
            if [[ -S "$VM_DIR/$1.serial" ]]; then
                print_status "INFO" "Connecting to $1 console (Ctrl+C to detach)..."
                socat - "unix-connect:$VM_DIR/$1.serial"
            else
                print_status "ERROR" "Serial socket not available"
                exit 1
            fi
